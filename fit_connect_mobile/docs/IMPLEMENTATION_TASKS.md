...
- [ ] **2.1 基本メッセージ機能**
  - [ ] メッセージ送信処理の実装（lib/features/messages/presentation/screens/message_screen.dart:204）
  - [ ] メッセージ受信のリアルタイム同期（Supabase Realtime）
  - [ ] 画像添付機能（最大3枚）
    - [ ] image_picker統合
    - [ ] Supabase Storage へのアップロード
    - [ ] 画像圧縮・リサイズ処理（最大1920x1080, 80%品質）
  - [ ] メッセージ一覧の取得とページネーション
  - [ ] 送信エラーハンドリング
...
- [ ] **2.2 タグ機能実装**
  - [ ] タグ入力補助UI改善
    - [ ] `#` 入力時の候補表示ロジック
    - [ ] リアルタイムフィルタリング（#食 → 食事関連タグのみ）
    - [ ] 1メッセージ1タグ制限のバリデーション
  - [ ] タグデータモデル定義
    - [ ] カテゴリタグ: `#食事`, `#運動`, `#体重`
    - [ ] 詳細タグ: `#食事:朝食`, `#運動:筋トレ` など
  - [ ] メッセージ送信時のタグ保存

- [ ] **2.3 Edge Functions作成**
  - [ ] `supabase/functions/parse-message-tags/index.ts`
    - [ ] タグ解析ロジック（正規表現: `/#(食事|運動|体重)(?::(.+?))?(?:\s|$)/`）
    - [ ] タグからメモ部分の抽出
    - [ ] Database Webhook設定（messages INSERT時）
  - [ ] `supabase/functions/create-meal-record/index.ts`
    - [ ] 食事タイプの判定（朝食/昼食/夕食/間食）
    - [ ] meal_recordsテーブルへの挿入
    - [ ] message_id紐付け、source='message'設定
  - [ ] `supabase/functions/create-weight-record/index.ts`
    - [ ] 体重値の抽出（例: "65.2kg" → 65.2）
    - [ ] weight_recordsテーブルへの挿入
    - [ ] 目標達成判定の呼び出し
  - [ ] `supabase/functions/create-exercise-record/index.ts`
    - [ ] 運動タイプの判定（筋トレ/有酸素）
    - [ ] exercise_recordsテーブルへの挿入
  - [ ] Edge Functionsのデプロイとテスト
...
- [ ] **3.1 体重記録機能**
  - [ ] 実データ取得と表示（lib/features/weight_records/presentation/screens/weight_record_screen.dart）
  - [ ] 折れ線グラフ実装（fl_chart使用）
    - [ ] 期間フィルタ対応（今週/今月/3ヶ月/全期間）
    - [ ] 目標体重を点線で表示
    - [ ] データポイントタップで詳細表示
    - [ ] 縦軸の自動調整
  - [ ] 統計カード表示
...
- [ ] **3.2 食事記録機能**
  - [ ] 実データ取得と表示（lib/features/meal_records/presentation/screens/meal_record_screen.dart）
  - [ ] GitHub草スタイルカレンダー実装
    - [ ] 記録数に応じた緑の濃淡（AppColors.grassLevel0-3使用）
    - [ ] 各日に記録数を数字で表示（③②①）
    - [ ] 3食: 濃い緑 #26A641
    - [ ] 2食: 中くらいの緑 #39D353
    - [ ] 1食: 薄い緑 #9BE9A8
...
- [ ] **3.3 運動記録機能**
  - [ ] 実データ取得と表示（lib/features/exercise_records/presentation/screens/exercise_record_screen.dart）
  - [ ] タイプフィルタ（すべて/筋トレ/有酸素）
  - [ ] 統計サマリー（総回数/タイプ別回数）
  - [ ] 週間カレンダー実装
    - [ ] 💪 筋トレアイコン表示
...
- [ ] **4.4 ホーム画面「本日のサマリー」実データ化**
  - [ ] 食事記録数の取得（本日の記録数 / 3食）
  - [ ] 運動記録数の取得（今週の運動回数 / 7日）
  - [ ] 体重の最新データ取得
    - [ ] 本日測定済み: 体重 + 測定時刻 + 前日比
    - [ ] 本日未測定: 最新体重 + 測定日 + 「本日未測定」表示
...
- [ ] **グラフ実装（fl_chart）**
  - 現在: lib/features/weight_records/presentation/screens/weight_record_screen.dart:143にプレースホルダー
  - LineChart ウィジェット実装
  - 期間フィルタ対応（今週/今月/3ヶ月/全期間）
  - 目標体重の点線表示
  - データポイントのタップ処理
...
- [ ] **GitHub草スタイルカレンダー実装**
  ...
  - カスタムウィジェット作成
  - 記録数カウントロジック
  - 色分けロジック（AppColors.grassLevel0-3）
  - 日付タップで詳細表示
...
1. **DBマイグレーション実行** → `supabase/migrations/` 確認・作成
2. **データモデル一括作成** → 全モデルファイル作成 → build_runner実行
3. **Riverpodプロバイダー作成** → 体重記録から開始（最もシンプル）
4. **体重記録画面の実データ化** → モックデータ削除 → Supabase統合
5. **メッセージリアルタイム機能** → Supabase Realtime統合
6. **Edge Functions作成** → タグ解析と記録作成の自動化
7. **グラフ実装** → fl_chartで体重折れ線グラフ
8. **目標達成機能** → Database Function + 達成画面 + 演出
